...
function getConversationOrderState(conversationId) {
  ...
  return {
    ...
    order_variant_id: row.order_variant_id ?? null,
...
function hardResetConversationOrderState(conversationId) {
  db.prepare(
    `
      UPDATE channel_conversations
      SET
...
function saveConversationOrderState(conversationId, patch) {
  ...
  db.prepare(
    ...
    next.order_variant_id ?? null,
    next.selected_size ?? null,
    next.selected_color ?? null,
